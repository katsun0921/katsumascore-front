// ISR: revalidate 60s — 現状運用: WP category として管理 (将来 /company/{slug} に移行予定)
...
const CompanyPage = ({ categoryName, slug, posts, currentPage, totalPages, locale }: CompanyPageProps) => {
  ...
  const handlePageChange = (page: number) => {
    const base = getEntityUrl('company', slug, loc);
  ...
  return (
    <I18nProvider locale={loc}>
      <Head>
        ...
        <meta name='description' content={`${categoryName}配給作品一覧 — スコアで選ぶ`} />
      </Head>
      <ListTemplate
        categoryName={categoryName}
        categoryDescription={`${categoryName}の配給作品を、スコアで選ぶ`}
        posts={sortedPosts}
