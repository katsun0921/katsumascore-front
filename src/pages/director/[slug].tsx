...
import { getEntityUrl } from '@/lib/route';
...
const DirectorPage = ({ categoryName, slug, posts, currentPage, totalPages, locale }: DirectorPageProps) => {
  ...
  const handlePageChange = (page: number) => {
    const base = getEntityUrl('director', slug, loc);
  ...
  return (
    <I18nProvider locale={loc}>
      <Head>
        ...
        <meta name='description' content={`${categoryName}監督作品一覧 — スコアで選ぶ`} />
      </Head>
      <ListTemplate
        categoryName={categoryName}
        categoryDescription={`${categoryName}の監督作品を、スコアで選ぶ`}
        posts={sortedPosts}
...
export const getStaticProps: GetStaticProps<DirectorPageProps> = async ({ params, locale }) => {
