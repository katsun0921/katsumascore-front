...
const FranchisePagedPage = ({ categoryName, slug, posts, currentPage, totalPages, locale }: FranchisePagedProps) => {
  ...
  const handlePageChange = (page: number) => {
    const base = getTaxonomyUrl('franchise', slug, loc);
  ...
  return (
    <I18nProvider locale={loc}>
      <Head>
        ...
        <meta name='description' content={`${categoryName}シリーズの記事一覧 — スコアで選ぶ`} />
      </Head>
      <ListTemplate
        categoryName={categoryName}
        categoryDescription={`${categoryName}シリーズの作品を、スコアで選ぶ`}
        posts={sortedPosts}
...
export default FranchisePagedPage;
...
export const getStaticProps: GetStaticProps<FranchisePagedProps> = async ({ params, locale }) => {
