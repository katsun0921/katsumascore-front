...
const AnimeIndexPage = ({
  ...
  const canonicalUrl = `${SITE_URL}${getPostTypeArchiveUrl({ type: 'anime', lang: loc, page: 1 })}`;
  ...
  const handlePageChange = (page: number) => {
    void router.push(
      getPostTypeArchiveNextPath({ type: 'anime', lang: loc, page }),
      getArchiveUrl(page),
...
export const getStaticProps: GetStaticProps<AnimeIndexProps> = async ({ locale }) => {
  ...
  if ('fetchFailed' in data) {
    const fallback = buildEmptyCategoryListPage(WP_ANIME_CATEGORY_SLUG, currentLocale);
    return {
      props: {
