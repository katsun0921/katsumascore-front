// ISR: revalidate REVALIDATE_DAILY — ドラマカテゴリ記事一覧（/drama, /en/drama）。1ページ目は ISR、ページ変更・フィルタ変更時のみ CSR（/api/category-filter-posts）で取得。
...
const DramaIndexPage = ({
  ...
  const handlePageChange = (page: number) => {
    void router.push(
      getPostTypeArchiveNextPath({ type: 'drama', lang: loc, page }),
      getArchiveUrl(page),
...
export const getStaticProps: GetStaticProps<DramaIndexProps> = async ({ locale }) => {
  ...
  const data = await loadCategoryListPage(WP_DRAMA_CATEGORY_SLUG, currentLocale, 1);
  ...
  if ('fetchFailed' in data) {
    const fallback = buildEmptyCategoryListPage(WP_DRAMA_CATEGORY_SLUG, currentLocale);
    return {
      props: {
