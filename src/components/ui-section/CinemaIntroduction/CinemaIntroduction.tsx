...
export const CinemaIntroduction = ({
  ...
  return (
    <section>
      <h2 className='font-heading text-h3 font-bold leading-[1.5] text-[var(--color-text-primary)] mb-3'>
        {text(['heading', 'prefix'])}
        {title}
        {text(['heading', 'suffix'])}
      </h2>
      {isShowing ? (
        <>
          <p className='text-body leading-[1.7] text-[var(--color-text-primary)] mb-4'>
            {text(['showing', 'bodyPrefix'])}
            {title}
            {text(['showing', 'bodySuffix'])}
          </p>
          {cinemaUrl && (
            <Link href={cinemaUrl} target='_blank' rel='noopener noreferrer' className='text-[var(--color-primary)] no-underline hover:underline'>
      ...
      <p className='text-ui text-[var(--color-text-secondary)] mt-4 font-bold'>
        ...
        {officialUrl && (
          <>
            {' '}
            <Link href={officialUrl} target='_blank' rel='noopener noreferrer' className='text-[var(--color-primary)] no-underline hover:underline'>
