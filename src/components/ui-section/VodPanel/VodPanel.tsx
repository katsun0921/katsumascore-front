  ...
  if (props.variant === 'cinema') {
    return (
      <div className='mt-[var(--space-32)] flex items-center gap-[var(--space-12)] rounded-[6px] border-l-[4px] border-l-[var(--color-category)] bg-[var(--color-bg-muted)] px-[var(--space-16)] py-[var(--space-12)]'>
        <span className='shrink-0 rounded-[4px] bg-[var(--color-category)] px-[var(--space-12)] py-[var(--space-4)] text-[var(--font-size-caption-pc)] font-[var(--font-weight-bold)] text-[var(--color-text-inverse)]'>
          {props.badgeLabel}
        </span>
        <p className='m-0 text-[var(--font-size-ui-pc)] text-[var(--color-text-secondary)]'>{props.note}</p>
      </div>
    )
  }
