...
> v4.5 ― Claude Code ベストプラクティスに基づく運用ガイドを追加
> 2026年5月16日
...
**IMPORTANT: 1ディレクトリに配置するコンポーネントは1つのみ。**
...
`src/libs` 配下の `.ts` ファイルでは、**振る舞いを持つ単位**（`export` する関数、クラスのメソッド、モジュール内のローカル関数で他から参照される処理の塊）ごとに、その直前に**説明コメント**を付ける。

| 項目 | ルール |
|------|--------|
| 形式 | JSDoc（`/** ... */`）を推奨 |
| 言語 | 日本語 |
| 内容 | 何をするか、主な引数・戻り値、優先順位やデフォルトなど呼び出し側が誤用しにくい情報 |
| ファイル全体 | モジュールの役割はファイル先頭のブロックコメントでまとめてよい（`import` の直後でも可） |
| 対象外 | 型・定数のみで関数・メソッドを持たないファイル |

詳細なファイル役割の一覧は `src/libs/CLAUDE.md` を参照する。
